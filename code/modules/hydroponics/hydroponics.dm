...
/obj/machinery/hydroponics/examine_more(user)
	...
	if(myseed && (in_range(user, src) || isobserver(user)))
		...
		switch(src.myseed.potency)	// Check potency
			...
			if(81 to 100)
				. += span_nicegreen("The harvest's size is huge and unmistakable!")
		switch(src.myseed.yield) 	// Check yield
			...
			if(8 to 10)
				. += span_nicegreen("The plant is high yielding and fertile!")
		switch(src.myseed.production) // Check production speed
			if(1 to 3)
				. += span_nicegreen("The plant'll be ready for harvest within a minute once it's matured!")
			if(4 to 7)
		...
		switch(src.myseed.endurance)	// Check endurance
			...
			if(90 to 100)
				. += span_nicegreen("The plant's endurance is herculean, capable of surviving far longer than any man!")
		switch(src.myseed.lifespan)		// Checks lifespan
			...
			if(90 to 100)
				. += span_nicegreen("The plant's lifespan is forevermore. Treat it well, and it will not abandon you.")
		switch(src.myseed.instability)		// Checks Instability
			if(0 to 20)
				. += span_nicegreen("The plant's stability is solid, the foundation secure.")
			if(21 to 40)
		...
		switch(src.myseed.weed_rate)		//Checks Weed Growth Rate
			if(0 to 2)
				. += span_nicegreen("The weed growth around the plant appears to be of a miniscule amount...")
			if(3 to 5)
		...
		switch(src.myseed.weed_chance)		//Checks weed growth chance
			if(0 to 20)
				. += span_nicegreen("... and the chance of the weeds to grow are highly unlikely.")
			if(21 to 40)
