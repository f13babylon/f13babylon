	...
	//STATE TRIGGERS
		...
		for(var/V in listeners)
			...
			if (E.phase == 3)
				...
				addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), C, "<span class='notice'>You whisper your triggers to [(E.lewd?"Your [E.enthrallGender]":"[E.master]")].</span>"), 5)
				...
				if(!E.lewd)
					...
					if (E.mental_capacity >= 5)
						...
						if ((findtext(trigger2, custom_words_words)))
							...
							E.mental_capacity -= 5
							addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), H, "<span class='notice'>[(E.lewd?"your [E.enthrallGender]":"[E.master]")] whispers you a new trigger.</span>"), 5)
			...
			if(E.phase == 3)
				if (get_dist(user, H) > 1)//Requires user to be next to their pet.
					...
					if (E.mental_capacity >= 200)
						...
						addtimer(CALLBACK(GLOBAL_PROC, PROC_REF(to_chat), H, "<span class='notice'>[(E.lewd?"Your [E.enthrallGender]":"[E.master]")] whispers you a new objective.</span>"), 5)
