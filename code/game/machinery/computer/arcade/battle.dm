...
/obj/machinery/computer/arcade/battle/Topic(href, href_list)
	...
	if (!blocked && !gameover)
		if (href_list["attack"])
			...
			playsound(loc, 'sound/arcade/hit.ogg', 10, 1, extrarange = -3, falloff = 10)
			updateUsrDialog()
			if(turtle > 0)
				turtle--
			...
			playsound(loc, 'sound/arcade/heal.ogg', 10, 1, extrarange = -3, falloff = 10)
			updateUsrDialog()
			turtle++
			...
			playsound(loc, 'sound/arcade/mana.ogg', 10, 1, extrarange = -3, falloff = 10)
			player_mp += chargeamt
			if(turtle > 0)
				turtle--
...
/obj/machinery/computer/arcade/battle/proc/arcade_action(mob/user)
	if ((enemy_mp <= 0) || (enemy_hp <= 0))
		...
		playsound(loc, 'sound/arcade/boom.ogg', 150, 1, extrarange = -3, falloff = 10)
		player_hp -= boomamt

	else if ((enemy_mp <= 5) && (prob(70)))
		var/stealamt = rand(2,3)
		temp = "[enemy_name] steals [stealamt] of your power!"
		playsound(loc, 'sound/arcade/steal.ogg', 10, 1, extrarange = -3, falloff = 10)
		player_mp -= stealamt
		updateUsrDialog()
		...
		if (player_mp <= 0)
			...
			playsound(loc, 'sound/arcade/lose.ogg', 10, 1, extrarange = -3, falloff = 10)
			if(obj_flags & EMAGGED)
				usr.gib()
