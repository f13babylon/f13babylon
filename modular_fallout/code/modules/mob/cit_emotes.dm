...
/datum/emote/living/scream/run_emote(mob/living/user, params) //I can't not port this shit, come on.
	...
	if(!user.is_muzzled() && !miming)
		...
		if(issilicon(user))
			sound = 'modular_fallout/sound/voice/scream_silicon.ogg'
			if(iscyborg(user))
				...
				S.cell.use(200)
		if(ismonkey(user))
			sound = 'modular_fallout/sound/voice/scream_monkey.ogg'
		if(istype(user, /mob/living/simple_animal/hostile/gorilla))
			sound = 'sound/creatures/gorilla.ogg'
		if(ishuman(user))
			user.adjustOxyLoss(5)
			sound = pick('modular_fallout/sound/voice/scream_m1.ogg', 'modular_fallout/sound/voice/scream_m2.ogg')
			if(user.gender == FEMALE)
				sound = pick('modular_fallout/sound/voice/scream_f1.ogg', 'modular_fallout/sound/voice/scream_f2.ogg')
			if(is_species(user, /datum/species/jelly))
				if(user.gender == FEMALE)
					sound = pick('modular_fallout/sound/voice/scream_jelly_f1.ogg', 'modular_fallout/sound/voice/scream_jelly_f2.ogg')
				else
					sound = pick('modular_fallout/sound/voice/scream_jelly_m1.ogg', 'modular_fallout/sound/voice/scream_jelly_m2.ogg')
			...
			if(is_species(user, /datum/species/lizard))
				sound = 'modular_fallout/sound/voice/scream_lizard.ogg'
			if(is_species(user, /datum/species/skeleton))
				sound = 'modular_fallout/sound/voice/scream_skeleton.ogg'
			if (is_species(user, /datum/species/fly) || is_species(user, /datum/species/insect))
				sound = 'modular_fallout/sound/voice/scream_moth.ogg'
		if(isalien(user))
...
/datum/emote/living/snap/run_emote(mob/living/user, params)
	...
	if(user.nextsoundemote >= world.time)
		return
	user.nextsoundemote = world.time + 7
	playsound(user, 'modular_fallout/sound/voice/snap.ogg', 50, 1, -1)
...
/datum/emote/living/snap2/run_emote(mob/living/user, params)
	...
	if(user.nextsoundemote >= world.time)
		return
	user.nextsoundemote = world.time + 7
	playsound(user, 'modular_fallout/sound/voice/snap2.ogg', 50, 1, -1)
...
/datum/emote/living/snap3/run_emote(mob/living/user, params)
	...
	if(user.nextsoundemote >= world.time)
		return
	user.nextsoundemote = world.time + 7
	playsound(user, 'modular_fallout/sound/voice/snap3.ogg', 50, 1, -1)
